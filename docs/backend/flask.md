...
| 对比项 | Flask | FastAPI |
|--------|-------|---------|
| 异步支持 | 需额外配置（Flask 2.0+） | 原生 async/await |
| 自动文档 | 需 Flask-RESTX | 内置 Swagger / ReDoc |
| 类型校验 | 手动校验或 Marshmallow | Pydantic 自动校验 |
| 性能 | 中等 | 高 |
| 适用场景 | 小型项目、模板渲染 | API 服务、微服务、异步场景 |

**选型建议：**
- 需要服务端渲染或快速原型 -> **Flask**
- 纯 API 服务、需要自动文档和异步 -> **FastAPI**
- 已有 Flask 生态 -> 继续用 **Flask**，逐步迁移
