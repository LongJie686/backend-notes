...
事务 A 对 `id=20` 加了行级 X 锁，事务 B 想对整张表加表锁：

```sql
LOCK TABLES t WRITE;
```

如果没有意向锁，InnoDB 只能扫描整张表判断有没有行锁 -> 效率太低。

**解决：** 事务 A 加行锁前，先在表级别自动加 IX，事务 B 看到表上有 IX 就知道有行被锁了。
...
| IX | 意向排他锁 | `SELECT ... FOR UPDATE` / `UPDATE` / `DELETE` |
...
> 意向锁是 InnoDB 在表级别自动加的"标记"，用来告诉别人"我正在或即将对某些行加锁"，便于快速判断表锁冲突，你写 SQL 时不用管它
...
等值查询 + 唯一索引，事务 A 只锁 `id=20` 这一条记录（记录锁），不锁间隙。`id=22` 是不同记录，不受影响。
...
范围当前读 + RR 隔离级别，会触发间隙锁/临键锁。假设索引上有 10, 20, 30：

- 锁住 `(10, 20]`（临键锁）
- 锁住 `(20, 30)`（间隙锁）

`id=22` 落在锁定范围内，被间隙锁阻塞。

### 练习 3：为什么 UPDATE 有时只锁一行，有时锁很多？

**答案：锁的粒度取决于查询条件能否高效利用索引**

| 情况 | 示例 | 锁范围 |
|------|------|--------|
| 等值 + 唯一索引 | `WHERE id=20` | 锁单行 |
| 等值 + 普通索引 | `WHERE user_id=1001` | 锁多行 + 间隙 |
| 范围查询 | `WHERE id>10 AND id<25` | 锁范围 + 间隙 |
| 无索引/索引失效 | `WHERE name='张三'`（name 无索引） | 扫描全表，锁大量行 |
