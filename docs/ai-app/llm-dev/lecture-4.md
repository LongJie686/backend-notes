...
1. **Agent 的智能不等于模型智能** -- 更多是工程设计，核心循环是感知 -> 推理 -> 行动 -> 观察
2. **ReAct 范式提供推理+行动循环** -- Thought -> Action -> Observation 交替进行，避免盲目行动，出错了可以自我修正
3. **工具描述的清晰度直接决定 Agent 的效果** -- 模型根据工具描述决定调用哪个工具，描述模糊会导致错误调用
4. **一定要设最大步数限制** -- 防止无限循环，一般 5-10 步，Token 消耗和响应时间都需要控制
5. **Agent 适合有明确目标和工具的场景** -- 不是万能的，任务可拆解、工具可定义时效果最好

---

## 一、Agent 核心概念与 ReAct 范式
...
### 1. Agent 和普通对话的本质区别是什么？
Agent 能自主调用外部工具、多步推理、自我修正 -> 是感知-推理-行动循环 -> 普通对话只是输入到输出的单轮映射

### 2. Function Calling 的底层原理是什么？
模型在训练时学会了理解工具描述 -> 根据用户意图输出结构化的工具调用请求（名称+参数） -> 应用层负责实际执行并将结果返回模型

### 3. Agent 为什么容易陷入循环？
工具描述不清晰导致重复调用同一工具、没有终止条件、错误反馈不正确 -> 设最大步数、优化工具描述、明确终止条件、加错误重试

---

## 练习题（待完成）

- [ ] 练习1：实现一个带工具调用的 Agent：搜索网页、查询数据库、执行代码
- [ ] 练习2：用 LangGraph 实现多步工作流
- [ ] 练习3：实现错误处理和兜底策略
- [ ] 练习4：构建多 Agent 协作系统（研究员 + 写作 + 审核）
