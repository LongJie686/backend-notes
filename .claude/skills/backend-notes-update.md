...
|----------|---------|-------------|
| backend | docs/backend/ | 后端 |
| database | docs/database/ | 数据库 |
| recommend-system | docs/recommend-system/ | 推荐系统 |
| ai-app | docs/ai-app/ | AI 应用 |
| microservice | docs/microservice/ | 微服务 |
| architecture | docs/architecture/ | 架构设计 |
| high-concurrency | docs/high-concurrency/ | 高并发系统设计 |
| engineering | docs/engineering/ | 工程化 |
| big-data | docs/big-data/ | 大数据 |
...
sidebar: [
  {
    text: '数据库',                    // 分类名（中文）
    items: [
      { text: 'MySQL', link: '/database/mysql' },               // 普通页面
      {
        text: 'MySQL 学习笔记',         // 折叠分组（中文）
        collapsed: true,
        items: [
...
1. **课程讲次**: `L{N}: {中文短标题}`
   - 示例: `L1: 表结构设计`, `L2: 索引`, `L7: 慢查询优化`
2. **普通页面**: `{中文名}` 或 `{英文名}`（根据内容决定）
   - 示例: `Python`, `Redis`, `LLM 概述`
3. **折叠分组**: `{中文名}`
   - 示例: `MySQL 学习笔记`, `Agent 架构分析`
4. **分类分组 text**: `{中文分类名}`
   - 示例: `数据库`, `微服务`, `架构设计`
...
1. 先**重新读取** `config.mts`（Obsidian 可能自动修改过）
2. 找到对应的分类分组（text 匹配）
3. 在分类分组内找到或创建折叠子分组
4. 在折叠子分组的 items 数组末尾追加新条目
5. 如果分类分组之前没有 `collapsed: true`，但内容超过 3 个 items，应考虑改为折叠式

### 新增分类时的 sidebar 更新逻辑

1. 先**重新读取** `config.mts`
2. 在 sidebar 数组末尾追加新分组
3. 格式遵循现有分组风格（中文 text）
4. 同时检查 nav 数组是否需要更新
...
4. **重新读取** `docs/.vitepress/config.mts`（防止 Obsidian 同步冲突）
5. 在微服务分组的 items 中追加:
   `{ text: 'L3: 服务治理', link: '/microservice/lecture-3' }`
6. 确认修改无误
...
3. **重新读取** `docs/.vitepress/config.mts`
4. 在 sidebar 数组中追加新分组:
   ```typescript
   {
     text: '设计模式',
     items: [
       { text: '学习路线', link: '/design-pattern/' }
     ]
   }
   ```
...
2. **重新读取** `docs/.vitepress/config.mts`
3. 在数据库分组的 items 中追加:
...
2. **重新读取** `docs/.vitepress/config.mts`
3. 一次性更新 config.mts，追加所有新条目
4. 确认文件编号连续，sidebar 顺序正确
...
- [ ] config.mts 的 sidebar text 格式正确（`LN: 中文标题` 或 `中文标题`）
- [ ] config.mts 语法正确（TypeScript，注意逗号和括号）
- [ ] 折叠分组（collapsed）的 items 中，lecture 按编号顺序排列
- [ ] 没有重复的 sidebar 条目
- [ ] 编辑 config.mts 前必须重新读取，防止 Obsidian 自动同步导致的冲突
